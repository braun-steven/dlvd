...
def svm_loss_vectorized(W, X, y, reg, delta=1):
    ...
    correct_scores = scores[np.arange(num_train), y].reshape(-1,
                                                             1)  # using the fact that all elements in y are < C == num_classes
    mat = scores - correct_scores + delta
    mat[np.arange(
        num_train), y] = 0  # accounting for the j=y_i term we shouldn't count (subtracting 1 makes up for it since w_j = w_{y_j} in this case)
