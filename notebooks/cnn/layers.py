...
def affine_backward(dout, cache):
    ...
    dw = x.T.dot(dout).reshape(w.shape)
        ...
        for j in xrange(F):  # jth filter
            ...
            for k in xrange(Hp):
                ...
                for l in xrange(Wp):
                    ...
                    # update db[j]. Since the partial derivative of dout[i, j, k, l] w.r.t db[j] is always
                    db[j] += dout[i, j, k, l]
        ...
        for j in xrange(C):
            for k in xrange(Hp):
                ...
                for l in xrange(Wp):
                    ...
                    dx[i, j, hs:hs + HH, ws:ws + WW] += dout[i, j, k, l] * (np.abs(window - m) < 10 ** -7)
                    # End of your code *****************************
